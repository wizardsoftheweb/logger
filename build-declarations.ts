...
const tmp = path.join(__dirname, ".logsWithWinstonTmp");
...
/* tslint:disable-next-line:no-conditional-assignment */
while (match = importRegExp.exec(declaration + EOL + functionDeclaration)) {
    if (/^"\w/y.test(match[4])) {
...
// Type definitions for LogsWithWinston ${config.version.replace(/\.\d+$/, "")}
